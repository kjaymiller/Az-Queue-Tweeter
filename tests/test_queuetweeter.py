...
def test_send_next_message_calls_client_with_text(mocker, test_queue_tweeter):
    ...
    mocker.patch(
        "azqueuetweeter.twitter.tweepy.Client.create_tweet",
        return_value={"Response": 200},
    )
    test_queue_tweeter.send_next_message()
    azqueuetweeter.twitter.tweepy.Client.create_tweet.assert_called_with(text="test")


def test_preview_mode_never_calls_client_with_text(mocker, test_queue_tweeter):
    """tests create_tweet is called when we send_next_message"""
    mocker.patch("azqueuetweeter.twitter.tweepy.Client.create_tweet")
    mocker.patch("azqueuetweeter.twitter.tweepy.API.media_upload")
    mocker.patch(
        "azqueuetweeter.storage.azure.storage.queue.QueueClient.receive_message"
    )
    mocker.patch(
        "azqueuetweeter.storage.azure.storage.queue.QueueClient.delete_message"
    )

    test_queue_tweeter.send_next_message(preview_mode=True)
    azqueuetweeter.twitter.tweepy.Client.create_tweet.assert_not_called()
    azqueuetweeter.twitter.tweepy.API.media_upload.assert_not_called()
    azqueuetweeter.storage.azure.storage.queue.QueueClient.receive_message.assert_not_called()
    azqueuetweeter.storage.azure.storage.queue.QueueClient.delete_message.assert_not_called()
...
def test_adding_file_triggers_media_upload(mocker, test_queue_tweeter, test_media):
    ...
    mocker.patch(
        "azqueuetweeter.twitter.tweepy.API.media_upload", return_value=test_media
    )
    mocker.patch(
        "azqueuetweeter.twitter.tweepy.Client.create_tweet",
        return_value={"Response": 200},
    ...
    azqueuetweeter.twitter.tweepy.API.media_upload.assert_called_with(
        file=b"passing_in_bytes", filename="bytes.png"
    )
    azqueuetweeter.twitter.tweepy.Client.create_tweet.assert_called_with(
